...
const inspectExpression = `(() => {
  ...
  window.__atlasComputerRefs = elements;
  return {
    url: location.href,
...
const output = await withCdp(async (send) => {
  ...
  if (action === "navigate") {
    ...
    const clicked = await evaluate(send, `(() => { const e = window.__atlasComputerRefs?.[${ref}]; if (!e) return false; e.scrollIntoView({block:'center'}); e.click(); return true; })()`);
    ...
    const typed = await evaluate(send, `(() => { const e = window.__atlasComputerRefs?.[${ref}]; if (!e) return false; e.focus(); if ('value' in e) { e.value = ${text}; e.dispatchEvent(new Event('input',{bubbles:true})); e.dispatchEvent(new Event('change',{bubbles:true})); } else { e.textContent = ${text}; e.dispatchEvent(new Event('input',{bubbles:true})); } return true; })()`);
