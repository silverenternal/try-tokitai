...
impl CompressionStrategy for ZstdCompressor {
    ...
    fn compress(&self, data: &[u8]) -> Result<Vec<u8>, Box<dyn Error + Send + Sync>> {
        let compressed = zstd::encode_all(data, self.level).map_err(|e| format!("zstd compression failed: {}", e))?;
        Ok(compressed)
    }
...
impl CompressionStrategy for SnappyCompressor {
    ...
    fn decompress(&self, data: &[u8]) -> Result<Vec<u8>, Box<dyn Error + Send + Sync>> {
        let len = snap::raw::decompress_len(data).map_err(|e| format!("snappy decompress_len failed: {}", e))?;
...
impl CompressionStrategy for Lz4Compressor {
    ...
    fn compress(&self, data: &[u8]) -> Result<Vec<u8>, Box<dyn Error + Send + Sync>> {
        ...
        let compressed =
            lz4::block::compress(data, Some(mode), true).map_err(|e| format!("lz4 compression failed: {}", e))?;
        Ok(compressed)
    }
    ...
    fn decompress(&self, data: &[u8]) -> Result<Vec<u8>, Box<dyn Error + Send + Sync>> {
        ...
        let decompressed =
            lz4::block::decompress(data, None).map_err(|e| format!("lz4 decompression failed: {}", e))?;
        Ok(decompressed)
    }
}
...
mod tests {
    ...
    fn test_zstd_repetitive() {
        ...
        assert!(
            ratio < 0.1,
            "Zstd ratio should be < 0.1 for repetitive data, got {:.3}",
            ratio
        );
    ...
    fn test_snappy_repetitive() {
        ...
        assert!(
            ratio < 0.15,
            "Snappy ratio should be < 0.15 for repetitive data, got {:.3}",
            ratio
        );
        test_roundtrip(&compressor, data.as_bytes());
    }
    ...
    fn test_lz4_repetitive() {
        ...
        assert!(
            ratio < 0.15,
            "LZ4 ratio should be < 0.15 for repetitive data, got {:.3}",
            ratio
        );
        test_roundtrip(&compressor, data.as_bytes());
    }
    ...
    fn test_algorithm_id_roundtrip() {
        for id in [
            CompressionAlgorithmId::None,
            CompressionAlgorithmId::Zstd,
            CompressionAlgorithmId::Snappy,
            CompressionAlgorithmId::Lz4,
        ] {
            let val = id as u8;
