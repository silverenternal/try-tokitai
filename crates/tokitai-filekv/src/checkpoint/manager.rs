...
impl IncrementalCheckpointManager {
        ...
        for entry in fs::read_dir(&self.checkpoint_dir).map_err(FatalError::Io)? {
            ...
            if path.is_file() && path.extension().is_some_and(|e| e == "tmp") {
                if let Some(stem) = path.file_stem().and_then(|s| s.to_str()) {
                    if stem.ends_with(".ckpt") {
                        tracing::warn!("Cleaning up leftover checkpoint temp file: {}", path.display());
    ...
    fn load_checkpoint_file(&self, path: &Path) -> Result<IncrementalCheckpoint> {
        ...
        // Verify integrity by recalculating the content hash
        let expected_hash_hex = self.calculate_content_hash(&checkpoint)?;

        if checkpoint.content_hash != expected_hash_hex {
            error!(
                "Checkpoint data corruption detected. Checkpoint ID: {}, File: {}, Expected hash: {}, Actual hash: {}",
            ...
            return Err(FatalError::Corruption(format!(
                "Checkpoint '{}' hash mismatch - expected: {}, actual: {}. The checkpoint file may be corrupted.",
    ...
    /// Create an incremental checkpoint with automatic full checkpoint trigger
    ///
    /// This method works like `create_incremental_checkpoint`, but when the
    /// `full_checkpoint_interval` is reached, it automatically creates a full
    /// checkpoint using the provided state.
    ///
    /// # Arguments
    /// * `changes` - List of changes (PUT/DELETE/MODIFY operations)
    /// * `state` - Current full state (used when auto-triggering full checkpoint)
    /// * `description` - Optional description for this checkpoint
    ///
    /// # Returns
    /// * `Ok(CheckpointId)` - The ID of the created checkpoint (incremental or full)
    pub fn create_incremental_checkpoint_with_auto_full<K, V>(
        &mut self,
        changes: Vec<CheckpointEntry>,
        state: Option<&HashMap<K, V>>,
        description: Option<&str>,
    ) -> Result<CheckpointId>
    where
        K: Clone + ToString,
        V: Clone + AsRef<[u8]>,
    {
        // First create the incremental checkpoint
        let incr_id = self.create_incremental_checkpoint(changes, description)?;

        // Check if we should auto-trigger a full checkpoint
        if self.needs_full_checkpoint() {
            if let Some(state) = state {
                info!("Auto-triggering full checkpoint after incremental checkpoint");
                match self.create_full_checkpoint(state, Some("Auto-triggered by interval")) {
                    Ok(full_id) => {
                        info!("Successfully created auto full checkpoint: {}", full_id);
                        Ok(full_id)
                    }
                    Err(e) => {
                        warn!(
                ...
                warn!("Full checkpoint interval reached but no state provided for auto-trigger");
                Ok(incr_id)
            }
        } else {
            Ok(incr_id)
    ...
    pub fn compact(&mut self, keep_last_n: usize) -> Result<usize> {
    ...
    fn delete_checkpoint(&mut self, checkpoint_id: &str) -> Result<()> {
    ...
    /// Save checkpoint to disk using atomic write (temp + rename)
    fn save_checkpoint(&self, checkpoint: &IncrementalCheckpoint) -> Result<u64> {
        let path = self.get_checkpoint_path(&checkpoint.checkpoint_id);
        let tmp_path = path.with_extension("ckpt.tmp");

        let content = serde_json::to_vec_pretty(checkpoint)
            .map_err(|e| FatalError::Corruption(format!("Failed to serialize checkpoint: {}", e)))?;
    ...
    fn calculate_content_hash(&self, checkpoint: &IncrementalCheckpoint) -> Result<String> {
    ...
    fn find_base_full_checkpoint(&self, checkpoint: &IncrementalCheckpoint) -> Result<CheckpointId> {
