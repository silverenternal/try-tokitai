...
pub struct CacheWarmer {
    ...
    /// Last warming operation stats (GAP-M8: stats tracking)
    last_stats: parking_lot::Mutex<Option<CacheWarmingStats>>,
}
...
impl CacheWarmer {
    ...
    pub fn warm(&self, segments: &[Arc<SegmentFile>]) -> FileKVResult<CacheWarmingStats> {
        ...
        // GAP-M8: Save stats for later retrieval
    ...
    fn scan_segment_for_candidates(
    ...
    ) -> FileKVResult<Vec<WarmingCandidate>> {
        let mut candidates = Vec::new();
        ...
        if self.config.strategy == WarmingStrategy::Recent {
            // Start scanning from a position that covers the most recent entries
            // Use a more accurate estimate: assume average entry ~100 bytes (more conservative)
            // This ensures we scan enough entries to find the recent ones
            let avg_entry_size = 100u64;
            offset = size.saturating_sub(entry_count.saturating_mul(avg_entry_size));
        }
    ...
    pub fn stats(&self) -> CacheWarmingStats {
        self.last_stats.lock().clone().unwrap_or_default()
    }
}
