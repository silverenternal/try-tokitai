...
impl FileKVMetrics {
    ...
    pub fn record_compaction(&self, segments_merged: u64, bytes_written: u64, entries_removed: u64, tombstones_cleaned: u64) {
        self.compaction_runs.fetch_add(1, Ordering::Relaxed);
        self.compaction_bytes_written.fetch_add(bytes_written, Ordering::Relaxed);
        self.compaction_segments_merged.fetch_add(segments_merged, Ordering::Relaxed);
        self.compaction_entries_removed.fetch_add(entries_removed, Ordering::Relaxed);
        self.compaction_tombstones_cleaned.fetch_add(tombstones_cleaned, Ordering::Relaxed);
    }
...
impl MetricsSnapshot {
    ...
    pub fn print(&self) {
        ...
        println!("  Hits: {}, Misses: {}", self.bloom_filter_hits, self.bloom_filter_misses);
        println!();
        println!("Memory Usage:");
        println!("  MemTable: {} bytes ({:.2} MB)", 
                 self.memtable_bytes, self.memtable_bytes as f64 / 1024.0 / 1024.0);
        println!("  Cache: {} bytes ({:.2} MB)", 
                 self.cache_bytes, self.cache_bytes as f64 / 1024.0 / 1024.0);
        println!("  Bloom Filter: {} bytes ({:.2} MB)", 
                 self.bloom_filter_bytes, self.bloom_filter_bytes as f64 / 1024.0 / 1024.0);
        println!("  Total: {} bytes ({:.2} MB)", 
                 self.total_memory_bytes(), self.total_memory_bytes() as f64 / 1024.0 / 1024.0);
        println!();
        println!("Amplification:");
        println!("  Write Amplification Factor: {:.2}x", self.write_amplification_factor);
        println!("  User Bytes Written: {} ({:.2} MB)", 
                 self.user_bytes_written, self.user_bytes_written as f64 / 1024.0 / 1024.0);
        println!("  Total Bytes Written: {} ({:.2} MB)", 
                 self.total_bytes_written, self.total_bytes_written as f64 / 1024.0 / 1024.0);
        println!();
        println!("Compaction:");
...
impl PrometheusExporter {
    ...
    pub fn export(&self, snapshot: &MetricsSnapshot) {
        // Operation counters
        counter!("filekv_writes_total", "instance" => self.instance_id.clone())
            .absolute(snapshot.write_count);
        counter!("filekv_reads_total", "instance" => self.instance_id.clone())
            .absolute(snapshot.read_count);
        counter!("filekv_deletes_total", "instance" => self.instance_id.clone())
            .absolute(snapshot.delete_count);
        counter!("filekv_write_errors_total", "instance" => self.instance_id.clone())
            .absolute(snapshot.write_errors);
        counter!("filekv_read_errors_total", "instance" => self.instance_id.clone())
            .absolute(snapshot.read_errors);
        counter!("filekv_delete_errors_total", "instance" => self.instance_id.clone())
            .absolute(snapshot.delete_errors);
        
        // Cache metrics
        counter!("filekv_cache_hits_total", "instance" => self.instance_id.clone())
            .absolute(snapshot.cache_hits);
        counter!("filekv_cache_misses_total", "instance" => self.instance_id.clone())
            .absolute(snapshot.cache_misses);
        ...
        // Memory metrics
        gauge!("filekv_memtable_bytes", "instance" => self.instance_id.clone())
            .set(snapshot.memtable_bytes as f64);
        gauge!("filekv_cache_bytes", "instance" => self.instance_id.clone())
            .set(snapshot.cache_bytes as f64);
        ...
        // Flush metrics
        counter!("filekv_flush_total", "instance" => self.instance_id.clone())
            .absolute(snapshot.flush_count);
        counter!("filekv_flush_errors_total", "instance" => self.instance_id.clone())
