...
fn test_put_batch_basic() {
    ...
    for (key, expected_value) in &entries {
        ...
        assert_eq!(
            result.as_ref().unwrap_or_else(|| panic!("Key {} should have a value", key)),
            *expected_value
        );
    }
...
fn test_put_batch_empty() {
    ...
    kv.put_batch(&entries).expect("put_batch with empty entries should succeed");
...
fn test_put_batch_wal_recovery() {
    ...
    assert_eq!(value1.as_ref().expect("batch1_key1 value missing").as_ref(), b"batch1_value1".as_ref());
    assert_eq!(value2.as_ref().expect("batch1_key2 value missing").as_ref(), b"batch1_value2".as_ref());
}
...
fn test_put_batch_multiple() {
    ...
    let batch1 = vec![
        ("m_key1", b"m_value1".as_slice()),
        ("m_key2", b"m_value2".as_slice()),
    ];
...
fn test_put_batch_overwrite() {
    ...
    // Batch overwrite
    let entries = vec![
        ("ow_key1", b"new1".as_slice()),
        ("ow_key2", b"new2".as_slice()),
    ];
    ...
    // Verify new values
    assert_eq!(
        kv.get("ow_key1").expect("get ow_key1 should succeed").expect("ow_key1 value missing").as_ref(),
        b"new1"
    );
    assert_eq!(
        kv.get("ow_key2").expect("get ow_key2 should succeed").expect("ow_key2 value missing").as_ref(),
        b"new2"
    );
}
...
fn test_put_batch_single() {
    ...
    assert_eq!(result.as_ref().expect("single_key value missing").as_ref(), b"single_value");
}
