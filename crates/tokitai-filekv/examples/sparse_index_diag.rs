...
fn main() {
    ...
    let config = FileKVConfig {
        ...
        cache: tokitai_filekv::BlockCacheConfig {
            max_items: 50_000,
            ..Default::default()
        },
        ..Default::default()
    ...
    for &size in &test_sizes {
        ...
        let test_keys: Vec<String> = (0..size).map(|i| format!("key_{:08}", i * (num_keys / size))).collect();
    ...
    let test_keys: Vec<String> = (0..1000).map(|i| format!("key_{:08}", i * 10)).collect();
