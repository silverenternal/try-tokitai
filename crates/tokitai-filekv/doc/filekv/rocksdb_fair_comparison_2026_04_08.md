...
|------|--------|---------|------|
| **P50 延迟** | **62.37 µs** | 247.38 µs | **3.97x** |
| **QPS** | **16,034** | 4,056 | **3.95x** |
|  outliers | 6/100 (6%) | 12/100 (12%) | 更稳定 |

### 分析
- **FileKV 优势**: 3.97x 性能提升
- **原因**: 
  - 自适应 Bloom Filter 缓存 (L1 命中仅 35ns)
  - 三层缓存架构减少内存占用
  - 压缩 Bloom Filter 减少内存带宽
...
| 指标 | FileKV | RocksDB | 提升 |
|------|--------|---------|------|
| **P50 延迟** | **61.92 µs** | 600.07 µs | **9.69x** |
| **QPS** | **16,171** | 1,668 | **9.69x** |
| Outliers | 9/100 (9%) | 4/100 (4%) | RocksDB 更稳定 |

### 分析
- **FileKV 优势**: 9.69x 性能提升
- **原因**:
  - 稀疏索引 + 二分查找高效
  - Block Cache 命中率高
  - 自适应 Bloom Filter 快速过滤
...
|---------|--------|---------|----------|
| **Bloom Filter 负向查询** | 62.37 µs | 247.38 µs | **3.97x** |
| **完整 KV 查询 (热缓存)** | 61.92 µs | 600.07 µs | **9.69x** |
