    ...
    for &range_size in &range_sizes {
        ...
        group.bench_with_input(
            ...
            |b, &range_size| {
                ...
                b.iter(|| {
                    let start = start_idx.fetch_add(1, Ordering::Relaxed) % (num_keys - range_size);
...
fn bench_compaction(c: &mut Criterion) {
    ...
    group.bench_function("trigger_compaction", |b| {
        b.iter(|| {
            // Each iteration: create KV, write data, trigger compaction
            let (_temp_dir, kv) = setup_kv(quick_bench_config(&TempDir::new().unwrap()));
            ...
            // Get stats to trigger/verify compaction state
            black_box(kv.get_stats());
        });
    });
