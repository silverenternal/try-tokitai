...
fn main() -> Result<()> {
    ...
    let window = WindowBuilder::new()
        ...
        .with_window_icon(build_tokitai_window_icon())
        ...
        *control_flow = ControlFlow::Wait;
        ...
        match event {
            ...
            Event::UserEvent(DesktopEvent::NativeBrowserNavigated { url }) => {
                let script = format!(
                    "window.__TOKITAI_NATIVE_BROWSER_NAVIGATED__ && window.__TOKITAI_NATIVE_BROWSER_NAVIGATED__({url:?});"
                );
                if let Err(err) = webview.evaluate_script(&script) {
...
fn build_tokitai_window_icon() -> Option<Icon> {
    const SIZE: u32 = 64;
    let decoder =
...
(() => {{
  ...
  window.__TOKITAI_NATIVE_BROWSER__ = {{
    send(payload = {{}}) {{
