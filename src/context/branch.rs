...
use super::hash_chain::{HashChain, HashChainManager};
...
impl ContextBranch {
    ...
    pub fn from_file(file_path: &Path) -> Result<Self> {
        let content = std::fs::read_to_string(file_path)
            .with_context(|| format!("Failed to read branch file: {:?}", file_path))?;

        let mut branch: ContextBranch = serde_json::from_str(&content)
            .with_context(|| format!("Failed to parse branch file: {:?}", file_path))?;
    ...
    pub fn save(&self) -> Result<()> {
        ...
        let content = serde_json::to_string_pretty(&serializable)
            .with_context(|| format!("Failed to serialize branch: {}", self.branch_id))?;

        let branch_file = self.branch_dir.join("branch.json");
        std::fs::write(&branch_file, content)
            .with_context(|| format!("Failed to write branch file: {:?}", branch_file))?;

        Ok(())
    }

    /// 获取分支的哈希链
    pub fn load_hash_chain(&self) -> Result<Option<crate::context::hash_chain::HashChain>> {
        if self.hash_chain_file.exists() {
            let content = std::fs::read_to_string(&self.hash_chain_file)
                .with_context(|| format!("Failed to read hash chain file: {:?}", self.hash_chain_file))?;

            let chain: crate::context::hash_chain::HashChain = serde_json::from_str(&content)
                .with_context(|| format!("Failed to parse hash chain file: {:?}", self.hash_chain_file))?;
...
impl BranchManager {
    /// 创建分支管理器
    pub fn new<P: AsRef<Path>>(branches_dir: P) -> Result<Self> {
    ...
    /// 加载所有现有分支
    fn load_all_branches(&mut self) -> Result<()> {
        if !self.branches_dir.exists() {
            return Ok(());
        }

        for entry in std::fs::read_dir(&self.branches_dir)
            .with_context(|| format!("Failed to read branches directory: {:?}", self.branches_dir))?
        {
            let entry = entry?;
            let path = entry.path();
    ...
    pub fn create_branch(
        ...
        if self.branches.contains_key(branch_id) {
            anyhow::bail!("Branch already exists: {}", branch_id);
        }
        ...
        if let Some(parent) = self.branches.get(parent_branch) {
            if parent.hash_chain_file.exists() {
                ...
                std::fs::copy(&parent.hash_chain_file, &branch.hash_chain_file)
                    .with_context(|| format!("Failed to copy hash chain from parent: {}", parent_branch))?;
    ...
    pub fn remove_branch(&mut self, branch_id: &str) -> Result<()> {
        if !self.branches.contains_key(branch_id) {
            anyhow::bail!("Branch does not exist: {}", branch_id);
        }
        ...
        if branch_dir.exists() {
            std::fs::remove_dir_all(&branch_dir)
                .with_context(|| format!("Failed to remove branch directory: {:?}", branch_dir))?;
        }
...
mod tests {
    ...
    fn test_branch_metadata() {
        ...
        // 创建带元数据的分支
        {
            ...
            metadata.created_by = "test-user".to_string();
            metadata.purpose = Some("Testing metadata".to_string());
            metadata.tags = vec!["test".to_string(), "metadata".to_string()];
            metadata.auto_merge = true;
            metadata.merge_strategy = MergeStrategy::FastForward;
            metadata.ttl_hours = Some(24);
