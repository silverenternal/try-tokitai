...
pub trait StorageLayer {
    /// 存储内容
    fn store(&self, content: &[u8], metadata: &ContentMetadata) -> Result<StoredItem>;
...
impl TransientLayer {
    pub fn new<P: AsRef<Path>>(dir: P) -> Result<Self> {
        ...
        std::fs::create_dir_all(&dir)
            .with_context(|| format!("Failed to create transient directory: {:?}", dir))?;
        Ok(Self { dir })
    }
    ...
    pub fn cleanup(&self) -> Result<()> {
        if self.dir.exists() {
            std::fs::remove_dir_all(&self.dir)
                .with_context(|| format!("Failed to cleanup transient directory: {:?}", self.dir))?;
...
impl StorageLayer for TransientLayer {
    fn store(&self, content: &[u8], metadata: &ContentMetadata) -> Result<StoredItem> {
        ...
        std::fs::write(&content_path, content)
            .with_context(|| format!("Failed to write transient content: {:?}", content_path))?;
    ...
    fn retrieve(&self, id: &str) -> Result<Vec<u8>> {
        ...
        std::fs::read(&content_path)
            .with_context(|| format!("Failed to read transient content: {:?}", content_path))
    }
...
impl ShortTermLayer {
    pub fn new<P: AsRef<Path>>(dir: P, max_rounds: usize) -> Result<Self> {
        let dir = dir.as_ref().to_path_buf();
        std::fs::create_dir_all(&dir)
            .with_context(|| format!("Failed to create short-term directory: {:?}", dir))?;
    ...
    /// 自动裁剪，保留最近 N 轮
    pub fn trim(&self) -> Result<Vec<String>> {
    ...
    /// 获取所有项目的元数据
    pub fn get_all_metadata(&self) -> Result<Vec<ContentMetadata>> {
...
impl StorageLayer for LongTermLayer {
    ...
    fn retrieve(&self, id: &str) -> Result<Vec<u8>> {
        ...
        anyhow::bail!("Content not found in long-term layer: {}", id)
    }
