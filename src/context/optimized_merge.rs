    ...
    fn merge_semantic_block(
        ...
        match (source, target, base) {
            (Some(s), Some(t), Some(b)) if s.content == t.content && t.content == b.content => {
                // 三者相同
                Ok(SemanticMergeOutcome::Merged(s.clone()))
            }
            ...
            (Some(s), Some(t), Some(b)) if s.content == b.content => {
                // 只有目标修改
                Ok(SemanticMergeOutcome::Merged(t.clone()))
            }
            ...
            (Some(s), Some(t), Some(b)) => {
                ...
                if diff3_result.success {
                    ...
                    Ok(SemanticMergeOutcome::Merged(merged_block))
                } else {
                    // 有冲突
                    let mut merged_block = s.clone();
                    merged_block.content = diff3_result.merged_content;
                    ...
                    Ok(SemanticMergeOutcome::Conflict(merged_block, conflict))
                }
            }
            (Some(s), None, _) => Ok(SemanticMergeOutcome::Merged(s.clone())),
            (None, Some(t), _) => Ok(SemanticMergeOutcome::Merged(t.clone())),
            (None, None, _) | (Some(_), Some(_), None) => {
                // 边缘情况：都删除或无 base
                Ok(SemanticMergeOutcome::Merged(SemanticBlock {
...
pub enum SemanticMergeOutcome {
    ...
    Conflict(SemanticBlock, Conflict),
}
