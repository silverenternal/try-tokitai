...
impl ContentAddressableStorage {
    ...
    pub fn retrieve(&mut self, hash: &str) -> Result<Vec<u8>> {
        ...
        // 解压缩
        let content = if compressed_data.len() >= self.config.min_size {
...
impl SnapshotManager {
    ...
    pub fn create_snapshot(
        ...
        metadata.description = description.map(|s| s.to_string());
        metadata.total_changes = changes.len();
...
mod tests {
    ...
    fn test_snapshot_chain() {
        ...
        let snapshot1 = manager.get_snapshot(&snap1).unwrap();
