...
impl ParallelMerger {
        ...
        if let Ok(entries) = std::fs::read_dir(source_dir) {
            for entry in entries.filter_map(|e| e.ok()) {
                ...
                if !target_path.exists() {
                    // 目标不存在，直接复制
                    if let Ok(_) = std::fs::copy(&source_path, &target_path) {
    ...
    fn create_merged_item(
        ...
        from_branch: &str,
        to_branch: &str,
        decision: crate::context::branch::MergeDecision,
    ) -> MergedItem {
        MergedItem {
            ...
            merge_decision: match decision {
                crate::context::branch::MergeDecision::KeepSource => crate::context::graph::MergeDecision::KeepSource,
                crate::context::branch::MergeDecision::KeepTarget => crate::context::graph::MergeDecision::KeepTarget,
                crate::context::branch::MergeDecision::Combine => crate::context::graph::MergeDecision::Combine,
                crate::context::branch::MergeDecision::Discard => crate::context::graph::MergeDecision::Discard,
                crate::context::branch::MergeDecision::AIResolved => crate::context::graph::MergeDecision::AIResolved,
            },
        }
    }
    ...
    fn create_conflict(
        ...
        Conflict {
            ...
            item_id: file_name.to_string(),
            source_version: crate::context::graph::ConflictVersion {
                hash: source_hash.to_string(),
                content_path: PathBuf::new(),
                metadata: None,
            },
            target_version: crate::context::graph::ConflictVersion {
...
mod tests {
    ...
    fn test_parallel_merge_layers() {
        ...
        let mut source = ContextBranch::new(
            "source",
            "source",
            "main",
            source_dir,
        ...
        let result = ParallelMergeResult {
            merged_items: vec![
                MergedItem {
                    ...
                    merge_decision: crate::context::graph::MergeDecision::KeepSource,
                },
            ],
            conflicts: vec![],
            parallel: true,
