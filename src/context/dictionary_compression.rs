...
impl DictionaryCompressor {
    ...
    pub fn compress_with_dict(&self, data: &[u8], dictionary: &[u8]) -> Result<Vec<u8>> {
        ...
        // 使用 zstd 的字典压缩 API
        // 注意：zstd crate 的字典压缩需要通过 train_dictionary 或外部工具
        // 这里使用简化的方法：直接压缩，但使用字典作为预处理
    ...
    pub fn decompress(&self, data: &[u8]) -> Result<Vec<u8>> {
        ...
        if self.config.enable_dictionary {
            if let Some(ref dictionary) = self.dictionary {
                // 注意：这里不能使用 self.stats 因为 self 是不可变借用
                // 我们直接尝试解压，失败后降级到标准解压
            }
        }
...
mod tests {
    ...
    fn test_dictionary_compression() {
        ...
        let samples: Vec<Vec<u8>> = (0..50)
            .map(|i| {
                format!(
                    r#"{{"type": "context", "id": {}, "content": "This is a sample context with some repeated patterns..."}}"#,
                    i
                )
                .into_bytes()
            })
        ...
        // 测试压缩
        let test_data = b"{\"type\": \"context\", \"id\": 999, \"content\": \"Test data\"}";
    ...
    fn test_dictionary_vs_standard() {
        ...
        let samples: Vec<Vec<u8>> = (0..100)
            .map(|i| {
                format!(
                    r#"{{"session": "test", "message": {}, "text": "Common patterns in context data"}}"#,
                    i
                )
                .into_bytes()
            })
        ...
        // 测试数据
        let test_data =
            b"{\"session\": \"new\", \"message\": 101, \"text\": \"Common patterns in context data\"}"
                .to_vec();
    ...
    fn test_small_file_compression() {
        ...
        // 训练
        let samples: Vec<Vec<u8>> = (0..50)
            .map(|i| format!("Small context data number {}", i).into_bytes())
            .collect();
        let dictionary = compressor.train_dictionary(&samples).unwrap();
        compressor.set_dictionary(dictionary);

        // 测试小文件
        let small_data = b"Small context data number 999".to_vec();
