...
use crate::context::branch::MergeDecision;
...
impl ThreeWayMerger {
    ...
    fn three_way_merge_file(
        ...
        match (source, target, base) {
            ...
            // 其他边缘情况
            _ => self.handle_edge_case(file_name, source, target, base, layer_name, source_branch, target_branch),
        }
    }
...
mod tests {
    ...
    fn create_test_branch_with_files(id: &str, name: &str, parent: &str, files: &[(&str, &str)]) -> ContextBranch {
        let temp_dir = TempDir::new().unwrap();
        let branch_dir = temp_dir.path().join(id);
        let mut branch = ContextBranch::new(id, name, parent, branch_dir).unwrap();
    ...
    fn test_three_way_merge_no_conflict() {
        ...
        let base = create_test_branch_with_files("base", "base", "main", &[("file1.txt", "base")]);
        let source = create_test_branch_with_files("source", "source", "base", &[("file1.txt", "modified")]);
        let target = create_test_branch_with_files("target", "target", "base", &[("file1.txt", "base")]);
    ...
    fn test_three_way_merge_with_conflict() {
        ...
        let base = create_test_branch_with_files("base", "base", "main", &[("file1.txt", "base")]);
        let source = create_test_branch_with_files("source", "source", "base", &[("file1.txt", "source_change")]);
        let target = create_test_branch_with_files("target", "target", "base", &[("file1.txt", "target_change")]);
    ...
    fn test_three_way_merge_same_change() {
        ...
        let base = create_test_branch_with_files("base", "base", "main", &[("file1.txt", "base")]);
        let source = create_test_branch_with_files("source", "source", "main", &[("file1.txt", "same_change")]);
        let target = create_test_branch_with_files("target", "target", "main", &[("file1.txt", "same_change")]);
    ...
    fn test_merge_comparison() {
        ...
        let base = create_test_branch_with_files("base", "base", "main", &[
            ("file1.txt", "base"),
            ("file2.txt", "base2"),
        ]);

        let source = create_test_branch_with_files("source", "source", "main", &[
            ("file1.txt", "change"),
            ("file2.txt", "base2"),
        ]);

        let target = create_test_branch_with_files("target", "target", "main", &[
            ("file1.txt", "base"),
            ("file2.txt", "change2"),
        ]);
