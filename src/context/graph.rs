...
use super::hash_chain::{ChainNode, HashChainManager};
...
impl ContextGraph {
    ...
    pub fn checkout(&mut self, branch_id: &str) -> Result<()> {
        ...
        if branch.state != BranchState::Active {
            anyhow::bail!(
                "Cannot checkout to non-active branch: {} (state: {})",
                branch_id,
                branch.state
            );
        }
...
impl ContextGraphManager {
    ...
    pub fn new<P: AsRef<Path>>(graph_dir: P) -> Result<Self> {
        ...
        let graph = if graph_file.exists() {
            ...
            let content = std::fs::read_to_string(&graph_file)
                .with_context(|| format!("Failed to read graph file: {:?}", graph_file))?;

            serde_json::from_str(&content)
                .with_context(|| format!("Failed to parse graph file: {:?}", graph_file))?
        } else {
            // 创建新图
    ...
    fn reload_branches(&mut self) -> Result<()> {
        ...
        for entry in std::fs::read_dir(&branches_dir)
            .with_context(|| format!("Failed to read branches directory: {:?}", branches_dir))?
        {
            let entry = entry?;
            let path = entry.path();
    ...
    pub fn save(&self) -> Result<()> {
        ...
        let content = serde_json::to_string_pretty(&serializable)
            .with_context(|| "Failed to serialize context graph")?;

        std::fs::write(&graph_file, content)
            .with_context(|| format!("Failed to write graph file: {:?}", graph_file))?;
    ...
    pub fn create_branch(
        ...
        if !parent_branch.is_empty() && !self.graph.branches.contains_key(parent_branch) {
            anyhow::bail!("Parent branch does not exist: {}", parent_branch);
        }
