...
pub trait FileContextService {
    ...
    /// 获取摘要
    fn get_summary(&self, hash: &str) -> Result<Option<String>>;

    /// 获取蒸馏后的结构化摘要
    fn get_distilled_summary(&mut self, session: &str, hash: &str) -> Result<Option<DistilledSummary>>;

    /// 为云端裁剪内容（只返回摘要 + 哈希）
    fn trim_for_cloud(&mut self, session: &str) -> Result<Vec<CloudContextItem>>;

    /// 为云端准备最小化载荷（蒸馏摘要 + 哈希链）
    fn prepare_cloud_payload(&mut self, session: &str, query: Option<&str>) -> Result<CloudPayload>;
    ...
    /// 删除内容
    fn delete(&mut self, session: &str, hash: &str) -> Result<()>;

    /// 清理会话
    fn cleanup_session(&mut self, session: &str) -> Result<()>;
}
...
impl SessionContext {
    fn new(dirs: SessionDirs, max_rounds: usize) -> Result<Self> {
...
impl FileContextServiceImpl {
    /// 创建服务实例
    pub fn new<P: AsRef<Path>>(root: P, config: FileContextConfig) -> Result<Self> {
    ...
    /// 获取或创建会话上下文
    fn get_or_create_session(&mut self, session_id: &str) -> Result<&mut SessionContext> {
    ...
    /// 获取或创建哈希链管理器
    fn get_or_create_hash_chain(&mut self, session_id: &str) -> Result<&mut HashChainManager> {
...
impl FileContextService for FileContextServiceImpl {
    ...
    fn get_by_hash(&self, hash: &str) -> Result<Vec<u8>> {
        ...
        let content = if self.config.enable_mmap {
            ...
            std::fs::read(&content_path)
                .with_context(|| format!("Failed to read content: {:?}", content_path))?
        };
    ...
    fn get_summary(&self, hash: &str) -> Result<Option<String>> {
        ...
        if summary_path.exists() {
            let summary = std::fs::read_to_string(&summary_path)
                .with_context(|| format!("Failed to read summary: {:?}", summary_path))?;
            Ok(Some(summary))
        } else {
            Ok(None)
        }
    ...
    fn get_distilled_summary(&mut self, _session: &str, hash: &str) -> Result<Option<DistilledSummary>> {
        // 首先检查缓存
    ...
    fn create_snapshot(&mut self, session: &str) -> Result<String> {
        if self.config.enable_hash_chain {
            ...
            anyhow::bail!("Hash chain is not enabled")
        }
    }
