...
impl ContextLayer {
    /// 创建新的上下文层
    pub fn new(layer_type: ContextLayerType, storage_dir: PathBuf, ttl_seconds: u64) -> Result<Self> {
    ...
    /// 添加上下文项
    pub fn add_item(&mut self, item: UnifiedContextItem) -> Result<()> {
...
impl UnifiedContextManager {
    /// 创建新的统一上下文管理器
    pub fn new<P: AsRef<Path>>(data_dir: P, session_id: &str) -> Result<Self> {
    ...
    /// 从配置创建
    pub fn with_config<P: AsRef<Path>>(data_dir: P, session_id: &str, config: UnifiedManagerConfig) -> Result<Self> {
        let data_dir = data_dir.as_ref().to_path_buf();
        let session_dir = data_dir.join(session_id);
    ...
    /// 添加上下文项
    pub fn add_item(&mut self, content: String, layer: ContextLayerType, source: ContextSource, topic_tags: Vec<String>) -> Result<String> {
        let now = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .unwrap()
            .as_secs();
    ...
    /// 标记项为已确认
    pub fn confirm_item(&mut self, item_id: &str) -> Result<()> {
        // 在各层中查找并确认
