...
#[derive(Debug, Clone)]
pub struct AIStats {
    pub conflict_resolver: super::ResolverStats,
    pub purpose_inference: super::InferenceStats,
    pub merge_recommender: super::RecommenderStats,
    pub summarizer: super::SummarizerStats,
}
...
impl AIEnhancedContextManager {
    ...
    /// 废弃分支
    pub fn abort_branch(&mut self, branch_id: &str) -> Result<()> {
    ...
    /// 比较分支差异
    pub fn diff(&self, branch1: &str, branch2: &str) -> Result<BranchDiff> {
        self.inner.diff(branch1, branch2)
    }

    /// 查看分支历史
    pub fn log(&self, branch_id: &str, limit: usize) -> Result<Vec<ChainNode>> {
