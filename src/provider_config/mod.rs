...
impl ProviderManager {
    ...
    pub fn from_env_file(env_path: Option<&str>) -> Result<Self, String> {
        ...
        let current_index = providers.iter()
    ...
    fn parse_providers(env_content: &str) -> Result<Vec<ProviderConfig>, String> {
        ...
        if provider_names.is_empty() {
            ...
            // 创建默认供应商
            let name = props.get("AI_API_URL")
                .map(|url| {
                    if url.contains("ollama") { "ollama" }
...
"#;
        ...
        assert_eq!(providers[0].api_url, "https://ollama.com/v1/chat/completions");
