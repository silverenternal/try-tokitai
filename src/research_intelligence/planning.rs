...
use crate::atlas_core::{AtlasCore, ObjectType, RelationshipKind, ScientificObject};
use anyhow::Result;
use serde_json::json;
...
impl PlanningEngine {
    ...
    pub fn plan(&self, goal: ResearchGoalInput, actor: &str) -> Result<ScientificPlan> {
        ...
        goal_object
            .metadata
            .insert("constraints".into(), goal.constraints.clone());
        goal_object
            .metadata
            .insert("estimate".into(), serde_json::to_value(&estimate)?);
        ...
        for (object_type, label, description) in definitions {
            ...
            object
                .metadata
                .insert("research_goal_id".into(), json!(goal_object.id));
        ...
        let mut plan_object =
            ScientificObject::new("experiment-plan", format!("Plan: {}", goal.title), actor);
        ...
        plan_object
            .metadata
            .insert("nodes".into(), serde_json::to_value(&nodes)?);
    ...
    pub fn fork(&self, plan_object_id: &str, actor: &str) -> Result<ScientificObject> {
        self.core
            .fork(plan_object_id, actor, "planning alternative")
    }
...
fn planning_nodes(goal: &ResearchGoalInput) -> Vec<(&'static str, String, String)> {
    vec![
        (
            "research-question",
            format!("Question: {}", goal.title),
            "Falsifiable research question".into(),
        ),
        (
            "hypothesis",
            format!("Hypothesis: {}", goal.title),
            "Initial testable explanation; Agent must refine before execution".into(),
        ),
        (
            "paper-analysis",
            "Paper analysis".into(),
            "Literature and prior-art evidence requirements".into(),
        ),
        (
            "dataset-requirement",
            "Dataset selection".into(),
            "Dataset constraints, license, version and leakage checks".into(),
        ),
        (
            "method",
            "Method selection".into(),
            "Baseline and candidate method definitions".into(),
        ),
        (
            "risk-analysis",
            "Risk analysis".into(),
            "Failure probability, resources and mitigation".into(),
        ),
        (
            "execution-plan",
            "Execution graph".into(),
            "Runtime-independent execution DAG".into(),
        ),
        (
            "expected-result",
            "Expected results".into(),
            "Metrics, stopping criteria and expected contribution".into(),
        ),
        (
            "publication-target",
            goal.target_publication
                .clone()
                .unwrap_or_else(|| "Publication structure".into()),
            "Publication outline backed by planned evidence".into(),
        ),
    ]
}
