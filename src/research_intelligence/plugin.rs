...
pub trait Plugin: EventListener + Send + Sync {
    ...
    fn install(&self, _context: &PluginContext) -> Result<()> { Ok(()) }
    fn enable(&self, _context: &PluginContext) -> Result<()> { Ok(()) }
    fn disable(&self, _context: &PluginContext) -> Result<()> { Ok(()) }
    fn unload(&self, _context: &PluginContext) -> Result<()> { Ok(()) }
    fn runtime_adapters(&self) -> Vec<Arc<dyn RuntimeAdapter>> { Vec::new() }
...
impl std::fmt::Debug for PluginRegistry {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter
            ...
            .field("plugin_count", &self.plugins.read().map(|value| value.len()).unwrap_or(0))
            .finish()
    }
}
...
impl PluginRegistry {
    ...
    pub fn enable(&self, name: &str, context: &PluginContext) -> Result<()> {
        let mut plugins = self.plugins.write().map_err(|_| anyhow!("plugin registry lock poisoned"))?;
        let entry = plugins.get_mut(name).ok_or_else(|| anyhow!("unknown Atlas plugin"))?;
