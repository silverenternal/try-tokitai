...
impl RecommendationEngine {
    ...
    pub fn evaluate(&self, focus_object_id: Option<&str>, actor: &str) -> Result<Vec<Recommendation>> {
        ...
        if experiments.iter().any(|object| object.lifecycle == LifecycleState::Failed) {
            candidates.push(candidate(
        ...
        if experiments.iter().any(|object| object.lifecycle == LifecycleState::Completed)
            && !objects.iter().any(|object| object.object_type.0 == "visualization")
        {
            candidates.push(candidate(
        ...
        if evidence.len() >= 2 && !objects.iter().any(|object| object.object_type.0 == "publication") {
            candidates.push(candidate(
        ...
        for recommendation in &mut candidates {
            ...
            object.metadata.insert("recommendation".into(), serde_json::to_value(&recommendation)?);
...
fn candidate(
    ...
    let focus_key = focus.map(|object| object.id.as_str()).unwrap_or("workspace");
    ...
    Recommendation {
        ...
        evidence_object_ids: focus
            .map(|object| object.evidence.iter().map(|item| item.evidence_object_id.clone()).collect())
            .unwrap_or_default(),
        related_object_ids: focus.map(|object| vec![object.id.clone()]).unwrap_or_default(),
...
mod tests {
    ...
    fn recommendations_are_ranked_and_stable() {
        ...
        assert_eq!(
            core.list().unwrap().iter().filter(|object| object.object_type.0 == "recommendation").count(),
            first.len()
        );
    }
}
