...
impl std::fmt::Debug for RuntimeRegistry {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter
            ...
            .field("adapter_count", &self.adapters.read().map(|value| value.len()).unwrap_or(0))
            .finish()
    }
}
...
impl ExecutionEngine {
    ...
    pub fn register_runtime(&self, runtime: Arc<dyn RuntimeAdapter>, actor: &str) -> Result<ScientificObject> {
        ...
        object.metadata.insert("capabilities".into(), serde_json::to_value(runtime.capabilities())?);
        object.metadata.insert("available".into(), json!(runtime.available()));
    ...
    pub fn create_task(&self, spec: &ExecutionTaskSpec, actor: &str) -> Result<ScientificObject> {
        ...
        object.metadata.insert("spec".into(), serde_json::to_value(spec)?);
        object.metadata.insert("status".into(), json!(ExecutionStatus::Planned));
        object.runtime.requested_capabilities = spec.required_capabilities.iter().cloned().collect();
        object.rebuild_search_index();
    ...
    pub fn execute(&self, task_object_id: &str, actor: &str) -> Result<ExecutionObservation> {
        ...
        if !task.can_execute(actor) {
            return Err(anyhow!("actor is not allowed to execute this scientific object"));
        }
    ...
    pub fn record_observation(&self, observation: &ExecutionObservation, actor: &str) -> Result<()> {
    ...
    pub fn pause(&self, id: &str, actor: &str) -> Result<ScientificObject> {
        self.core.update(id, &json!({"metadata": {"status": "paused"}}), actor)
    }
    ...
    pub fn execute_parallel(&self, task_ids: &[String], actor: &str) -> Vec<Result<ExecutionObservation>> {
        ...
        task_ids.iter().map(|task_id| self.execute(task_id, actor)).collect()
    }
}
...
fn ensure_dependencies_completed(core: &AtlasCore, task: &ScientificObject) -> Result<()> {
    for relationship in task.relationships.iter().filter(|relationship| {
        ...
        if dependency.lifecycle != LifecycleState::Completed {
            return Err(anyhow!("execution dependency is not completed: {}", dependency.display_name));
        }
    }
    Ok(())
}
...
fn analyze_failure(message: &str) -> FailureAnalysis {
    ...
    let (category, strategy, retryable) = if lower.contains("out of memory") || lower.contains("oom") {
        (FailureCategory::MemoryOverflow, "reduce memory pressure or move to a larger runtime", true)
    } else if lower.contains("timeout") || lower.contains("timed out") {
        (FailureCategory::Timeout, "checkpoint, reduce task scope, or increase the runtime limit", true)
    } else if lower.contains("converg") || lower.contains("nan") {
        (FailureCategory::BadConvergence, "adjust optimizer, initialization, scale, or numerical tolerances", true)
    } else if lower.contains("accuracy") || lower.contains("metric gate") {
        (FailureCategory::LowAccuracy, "inspect errors and run a controlled ablation", true)
    } else if lower.contains("dataset") || lower.contains("data format") {
        (FailureCategory::InvalidDataset, "validate the immutable dataset manifest and split", false)
    } else if lower.contains("busy") || lower.contains("unavailable") {
        (FailureCategory::RuntimeBusy, "select a compatible available runtime", true)
    } else if lower.contains("not found") || lower.contains("missing") {
        (FailureCategory::MissingDependency, "install or configure the required capability", false)
    } else {
        (FailureCategory::RuntimeFailure, "inspect runtime evidence and revise the execution strategy", true)
    };
    FailureAnalysis {
        category,
...
mod tests {
    ...
    impl RuntimeAdapter for TestRuntime {
        fn runtime_object_id(&self) -> &str { "test-runtime" }
        fn capabilities(&self) -> BTreeSet<String> { BTreeSet::from(["python".into()]) }
    ...
    fn selects_runtime_and_records_execution_lifecycle() {
        ...
        engine.register_runtime(Arc::new(TestRuntime), "agent").unwrap();
        let task = engine
            .create_task(
                &ExecutionTaskSpec {
        ...
        assert_eq!(core.get(&task.id).unwrap().lifecycle, LifecycleState::Completed);
        assert!(core.list().unwrap().iter().any(|object| object.object_type.0 == "runtime"));
    }
}
