...
use super::tracing::{SpanStatus, TraceSpan, TracingRecorder};
...
impl ObservabilityTools {
    ...
    pub fn new(log_dir: &str) -> Result<Self, String> {
        ...
        fs::create_dir_all(&storage_dir).map_err(|e| format!("创建追踪目录失败：{}", e))?;
    ...
    fn load_traces_from_file(&self, file_path: &Path) -> Result<Vec<TraceSpan>, String> {
        ...
        let file = File::open(file_path).map_err(|e| format!("打开追踪文件失败：{}", e))?;
        ...
        for line in reader.lines() {
            ...
            let span: TraceSpan =
                serde_json::from_str(&line).map_err(|e| format!("解析追踪记录失败：{}", e))?;
            spans.push(span);
        }
    ...
    fn get_trace_files(&self) -> Result<Vec<PathBuf>, String> {
        ...
        for entry in
            fs::read_dir(&self.storage_dir).map_err(|e| format!("读取追踪目录失败：{}", e))?
        {
...
impl Default for ObservabilityTools {
    fn default() -> Self {
        Self::new(".tokitai/traces").unwrap_or_else(|_| Self::new("target/traces").unwrap())
    }
}
...
impl ObservabilityTools {
    ...
    pub fn export_traces(
        ...
        for span in &spans {
            let json =
                serde_json::to_string(span).map_err(|e| format!("序列化追踪数据失败：{}", e))?;
    ...
    pub fn cleanup_old_traces(&self, keep_days: Option<u32>) -> Result<Value, String> {
        ...
        for file_path in self.get_trace_files()? {
            ...
            if !has_recent {
                fs::remove_file(&file_path).map_err(|e| format!("删除追踪文件失败：{}", e))?;
                deleted += 1;
            }
        }
