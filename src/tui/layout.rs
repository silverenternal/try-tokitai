...
impl TuiLayout {
    pub fn calculate(
        ...
        if suggestions_h > 0 {
            constraints.push(Constraint::Length(suggestions_h));
        }
        if thinking_h > 0 {
            constraints.push(Constraint::Length(thinking_h));
        }
        ...
        let suggestions_area = if suggestions_h > 0 {
            let a = chunks[idx];
            idx += 1;
            a
        } else {
        ...
        let thinking_area = if thinking_h > 0 {
            let a = chunks[idx];
            idx += 1;
            a
        } else {
...
mod tests {
    ...
    fn test_layout_basic() {
        let layout = TuiLayout::calculate(Rect::new(0, 0, 100, 40), 3, 0, 0, 0);
    ...
    fn test_layout_with_suggestions() {
        let layout = TuiLayout::calculate(Rect::new(0, 0, 100, 40), 3, 0, 3, 0);
        assert_eq!(layout.suggestions_area.height, 3);
        assert_eq!(layout.chat_area.height, 33);
    }

    #[test]
    fn test_layout_with_reviewer_panel() {
        let layout = TuiLayout::calculate(Rect::new(0, 0, 100, 40), 3, 0, 0, 6);
        assert_eq!(layout.review_area.height, 6);
        assert_eq!(layout.chat_area.height, 30);
    }
}
