    ...
    fn render_block(
        ...
        match block {
            MessageBlock::ToolCall {
                ...
                let args_short = if args_str.chars().count() > 100 {
                    format!("{}...", args_str.chars().take(97).collect::<String>())
                } else {
                    args_str
                };
            ...
            MessageBlock::Subagent { record } => {
                ...
                if !record.purpose.is_empty() {
                    lines.push(Line::from(Span::styled(
                        format!("  {}", record.purpose),
                        Style::default().fg(Color::DarkGray),
                    )));
                }
                if !record.output.is_empty() {
                    for line in record.output.lines().take(6) {
                        lines.push(Line::from(Span::styled(
                            format!("  {}", line),
                            Style::default().fg(Color::Gray),
                        )));
            ...
            MessageBlock::Verification { report } => {
                lines.push(Line::from(vec![
                    Span::styled("[verify] ", Style::default().fg(Color::Yellow)),
                    Span::styled(
                ...
                for item in report.issues.iter().take(4) {
                    lines.push(Line::from(Span::styled(
                        format!("  - {}", item),
                        Style::default().fg(Color::DarkGray),
                    )));
