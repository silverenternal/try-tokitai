...
impl RegistryClient {
    ...
    pub async fn search(&self, query: &str) -> Result<SearchResults> {
        ...
        if response.status().is_success() {
            let results = response.json::<SearchResults>()
                .await
                .context("解析搜索结果失败")?;
            Ok(results)
        } else {
    ...
    pub async fn download(&self, tool_name: &str) -> Result<ToolPackage> {
        ...
        if response.status().is_success() {
            let package = response.json::<ToolPackage>()
                .await
                .context("解析工具包失败")?;
            Ok(package)
        } else {
...
impl ToolMarket {
    ...
    pub fn new(registry_config: Option<RegistryConfig>) -> Result<Self> {
        ...
        // 创建本地工具目录
        std::fs::create_dir_all(&local_tools_dir)
            .context("创建本地工具目录失败")?;
    ...
    pub async fn search(&self, query: &str) -> Result<()> {
        ...
        for (i, tool) in results.results.iter().enumerate() {
            ...
            println!("   版本：{} | 分类：{} | 标签：{}", 
                     tool.version, tool.category, tool.tags.join(", "));
        }
    ...
    pub async fn install(&self, tool_name: &str) -> Result<()> {
        ...
        std::fs::write(src_dir.join("lib.rs"), &package.source_code)
            .context("保存源代码失败")?;
        
        // 保存 Cargo.toml
        let cargo_toml = self.generate_cargo_toml(&package.metadata);
        std::fs::write(tool_dir.join("Cargo.toml"), cargo_toml)
            .context("保存 Cargo.toml 失败")?;
