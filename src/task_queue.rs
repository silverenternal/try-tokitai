...
impl TaskQueue {
    pub fn open(workspace: &Path) -> Result<Self> {
        ...
        let dir = workspace.join(".tokitai").join("tasks");
...
fn write_result_artifact(workspace: &Path, task: &BackgroundTask) -> Result<()> {
    ...
    let parent = path.parent().ok_or_else(|| anyhow!("task result path has no parent"))?;
    fs::create_dir_all(parent)?;
    let canonical_parent = parent.canonicalize()?;
    ...
    object.insert("schema_version".into(), json!("atlas.domain-action-result.v1"));
