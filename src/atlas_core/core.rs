...
impl AtlasCore {
    ...
    pub fn sync_external(&self, mut incoming: ScientificObject, actor: &str) -> Result<ScientificObject> {
    ...
    pub fn update(&self, id: &str, patch: &Value, actor: &str) -> Result<ScientificObject> {
        ...
        if !object.can_write(actor) {
            return Err(anyhow!("actor is not allowed to update this scientific object"));
        }
    ...
    pub fn fork(&self, id: &str, actor: &str, reason: &str) -> Result<ScientificObject> {
        ...
        if !source.can_read(actor) {
            return Err(anyhow!("actor is not allowed to fork this scientific object"));
        }
        ...
        self.relate(&fork.id, id, RelationshipKind::ForkOf, actor, BTreeMap::new())?;
    ...
    pub fn merge(
        ...
        if !source.can_read(actor) || !target.can_write(actor) {
            return Err(anyhow!("actor is not allowed to merge these scientific objects"));
        }
        let parent = target.version;
        target.metadata.extend(source.metadata.clone());
        target.tags.extend(source.tags.clone());
        for artifact in source.artifacts {
            if !target.artifacts.iter().any(|existing| existing.id == artifact.id) {
    ...
    pub fn compare(&self, id: &str, left: u64, right: u64) -> Result<ObjectComparison> {
        ...
        let keys = left_value
            ...
            .chain(right_value.as_object().into_iter().flat_map(|value| value.keys()))
            .cloned()
            .collect::<BTreeSet<_>>();
        for key in keys {
    ...
    pub fn relate(
        ...
        if !source.can_write(actor) || !target.can_write(actor) {
            return Err(anyhow!("actor is not allowed to relate these scientific objects"));
        }
        if let Some(existing) = self.store.list_relationships()?.into_iter().find(|relationship| {
...
mod tests {
    ...
    fn object_lifecycle_versions_and_relationships_are_persistent() {
        ...
        let first = core
            .create(ScientificObject::new("experiment", "Run A", "tester"), "tester")
            .unwrap();
        let second = core
            .create(ScientificObject::new("dataset", "Data A", "tester"), "tester")
            .unwrap();
        let updated = core
            .update(
                &first.id,
