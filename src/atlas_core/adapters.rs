...
pub fn domain_asset_object(asset: &DomainAsset, owner: &str) -> ScientificObject {
    ...
    object
        .metadata
        .insert("domain_id".into(), json!(asset.domain_id));
    object
        .metadata
        .insert("source_id".into(), json!(asset.source_id));
...
pub fn domain_task_object(task: &DomainTaskRecord) -> ScientificObject {
    let mut object =
        ScientificObject::new("experiment", task.intent_label.clone(), task.agent.clone());
    ...
    object
        .metadata
        .insert("domain_id".into(), json!(task.domain_id));
    object.metadata.insert("task_id".into(), json!(task.id));
    object.metadata.insert("status".into(), json!(task.status));
    object
        .metadata
        .insert("current_stage".into(), json!(task.current_stage));
    object
        .metadata
        .insert("parameters".into(), task.parameters.clone());
...
pub fn domain_workspace_object(workspace: &DomainWorkspace, owner: &str) -> ScientificObject {
    ...
    object
        .metadata
        .insert("domain_id".into(), json!(workspace.domain.metadata.id));
    object.metadata.insert(
        "domain_version".into(),
        json!(workspace.domain.metadata.version),
    );
    object
        .metadata
        .insert("workspace_revision".into(), json!(workspace.revision));
    object.ai_context = AgentContext {
        summary: format!(
            "Active {} scientific workspace",
            workspace.domain.metadata.label
        ),
...
pub fn legacy_research_object(
    object_type: &str,
    source: &Value,
    owner: &str,
) -> Option<ScientificObject> {
    ...
    object
        .metadata
        .insert("legacy_research_os_id".into(), json!(id));
    object
        .metadata
        .insert("legacy_payload".into(), source.clone());
    object.lifecycle = match source
        .get("status")
        .and_then(Value::as_str)
        .unwrap_or_default()
    {
...
fn stable_id(namespace: &str, source: &str) -> String {
    blake3::hash(format!("atlas:{namespace}:{source}").as_bytes()).to_hex()[..32].to_string()
}
