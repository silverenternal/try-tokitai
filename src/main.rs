...
// └─────────────────────────────────────────────────────────────────────────┘
//
// Host architecture is documented in README.md and docs/.
// ============================================================================
...
fn main() -> Result<()> {
    ...
    if use_tui {
        ...
        let log_file = std::fs::File::create(app_paths.tui_log_path()).unwrap_or_else(|_| {
            let fallback =
                std::env::temp_dir().join(format!("atlas_tui_{}.log", std::process::id()));
...
mod tests {
    ...
    fn test_file_operations_read_write() {
        ...
        let test_path = "/tmp/test_atlas.txt";
        let test_content = "Hello, Atlas!";
