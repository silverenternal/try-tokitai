...
const ACTIONS: &[ActionTemplate] = &[
    ...
    action(
        ...
        "container-snapshot",
        "Container Snapshot",
        "Read container identity, image, state, ports and names through the Atlas Docker provider.",
        "Docker Engine API",
        &[],
        false,
        NO_PARAMETERS,
        "container-snapshot",
        InvocationKind::Program(&["ps", "--all", "--format", "json"]),
    ),
    action(
        "distributed-systems",
        "image-snapshot",
        "Image Snapshot",
        "Read local image identities, tags, sizes and creation times through the Atlas Docker provider.",
        "Docker Engine API",
        &[],
        false,
        NO_PARAMETERS,
        "image-snapshot",
        InvocationKind::Program(&["image", "ls", "--format", "json"]),
    ),
    action(
...
fn action_probe(template: &ActionTemplate) -> super::adapters::SdkProbe {
    let required_programs: Option<&[&str]> = match (template.domain_id, template.id) {
        ...
        ("distributed-systems", _) => Some(&["docker"]),
