...
use super::model::{
    ...
    DomainVisualizationDescriptor, DomainWorkbenchDescriptor, DomainWorkbenchStageDescriptor,
    DomainWorkbenchToolDescriptor, DomainWorkspace, DomainWorkspaceSummary, ResearchDomainCatalog,
...
fn discover_assets(plugin: &DeclarativeDomainPlugin, root: &Path) -> Result<Vec<DomainAsset>> {
        ...
        .filter_map(Result::ok)
    {
        ...
        let is_action_result =
            action_domain.is_some() && relative_display.ends_with("/result.json");
...
mod tests {
    ...
    fn domain_action_result_is_isolated_and_visualizable() {
        ...
        let other = registry.workspace(&context, "computer-vision").unwrap();
        assert!(!other
            .assets
            .iter()
