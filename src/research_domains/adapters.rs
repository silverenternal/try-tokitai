...
fn parse_action_result(
    ...
    if value.get("schema_version").and_then(Value::as_str)
        != Some("atlas.domain-action-result.v1")
    {
    ...
    for key in ["task_id", "asset", "title", "program", "command", "log_path"] {
    ...
    for key in ["status", "exit_code", "started_at", "completed_at", "output_truncated"] {
        if let Some(field) = value.get(key) {
            table_rows.push(json!({"Field": key, "Value": display_json_value(field)}));
        }
    }
    if let Some(output) = value.get("output").and_then(Value::as_str) {
        document.metadata.insert("action_output".to_string(), json!(output));
...
fn append_action_result_value(
    ...
    match value {
        Value::Object(object) => {
            for (key, child) in object {
                ...
                if child.is_array() || child.is_object() {
                    ...
                    } else if let Some(node) = document.nodes.iter_mut().find(|node| node.id == parent) {
        ...
        Value::Array(values) => {
            for (index, child) in values.iter().take(MAX_DOMAIN_NODES.saturating_sub(document.nodes.len())).enumerate() {
                ...
                let id = format!("action-item:{}:{}", depth, stable_value_id(parent, &index.to_string()));
                ...
                document.nodes.push(node);
                document.edges.push(VisualizationEdge::new(
                    format!("action-edge:{}", stable_value_id(parent, &index.to_string())),
                    parent,
                    &id,
                    "contains",
                    "action-result",
