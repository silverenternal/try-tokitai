...
fn update_hypothesis_op(workspace_root: &Path, params: &Value) -> Result<Value> {
    ...
    if let Some(ref status_str) = status {
        ...
        if !matches!(
            status_str.as_str(),
            "draft" | "active" | "validated" | "refuted" | "abandoned"
        ) {
    ...
    let updates = HypothesisUpdate {
        ...
        parent_hypothesis_id: params
            .get("parent_hypothesis_id")
            .and_then(Value::as_str)
            .map(String::from),
...
fn create_evidence_op(workspace_root: &Path, params: &Value) -> Result<Value> {
    let kind = params.get("kind").and_then(Value::as_str).ok_or_else(|| {
        anyhow!("create_evidence requires kind (experimental, literature, artifact, benchmark)")
    })?;
    ...
    if !(0.0..=1.0).contains(&strength) {
        return Err(anyhow!(
            "Evidence strength must be between 0.0 and 1.0, got {}",
            strength
        ));
    }
    ...
    let source_path = params
        ...
        .and_then(Value::as_str)
        .map(String::from);
    let source_command = params
        .get("source_command")
        .and_then(Value::as_str)
        .map(String::from);
    let hypothesis_id = params
        .get("hypothesis_id")
        .and_then(Value::as_str)
        .map(String::from);
...
fn update_experiment_op(workspace_root: &Path, params: &Value) -> Result<Value> {
    ...
    if let Some(ref status_str) = status {
        if !matches!(
            ...
            return Err(anyhow!(
                "Invalid experiment status: {}. Must be planned, running, completed, or failed.",
                status_str
            ));
        }
    }
...
fn create_decision_op(workspace_root: &Path, params: &Value) -> Result<Value> {
    ...
    if !(0.0..=1.0).contains(&decision_score) {
        return Err(anyhow!(
            "decision_score must be between 0.0 and 1.0, got {}",
            decision_score
        ));
    }
...
fn create_memory_op(workspace_root: &Path, params: &Value) -> Result<Value> {
    ...
    if !(0.0..=1.0).contains(&importance) {
        return Err(anyhow!(
            "Memory importance must be between 0.0 and 1.0, got {}",
            importance
        ));
    }
...
fn update_publication_op(workspace_root: &Path, params: &Value) -> Result<Value> {
    ...
    if let Some(ref status_str) = status {
        ...
        if !matches!(
            ...
            return Err(anyhow!(
                "Invalid publication status: {}. Must be draft, review, ready, or published.",
                status_str
            ));
        }
    }
...
fn link_objects_op(workspace_root: &Path, params: &Value) -> Result<Value> {
    ...
    let edge_type = match relation {
        ...
        _ => {
            return Err(anyhow!(
                "Invalid relation: {}. Must be cites, uses, extends, or contradicts.",
                relation
            ))
        }
    };
