...
use super::diary::{
    create_diary_entry, create_diary_entry_with_source, list_diary_entries, DiaryEntryType,
};
use super::evidence::{create_evidence, list_evidence, EvidenceKind};
use super::experiment_lineage::{
    create_experiment, list_experiments, update_experiment, ExperimentUpdate,
};
...
pub fn ingest_domain_task(
    ...
    if (status == "failed" || status == "error") && previous_status.as_deref() != Some("failed") {
...
mod tests {
    ...
    fn ingests_successful_domain_task() {
        ...
        assert!(evidence.iter().any(|item| {
            item.kind == "benchmark"
                && item.source_command.as_deref()
                    == Some("python evaluate.py --checkpoint model.pth")
