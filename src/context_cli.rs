...
//!
//! 提供 Git 风格的平行上下文管理命令：
//! - `atlas ctx branch [name]` - 创建或列出分支
//! - `atlas ctx checkout <branch>` - 切换分支
//! - `atlas ctx merge <source> [target]` - 合并分支
//! - `atlas ctx abort <branch>` - 废弃分支
//! - `atlas ctx diff <branch1> [branch2]` - 查看差异
//! - `atlas ctx log [branch]` - 查看历史
//! - `atlas ctx time-travel <branch> <hash>` - 时间旅行
