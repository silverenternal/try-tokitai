...
| 首次请求延迟 | ~200ms | **~100ms** | **2x** |
| 流式首字节延迟 | ~150ms | **~50ms** | **3x** |
| 缓存容量 | 100 条目 | **200 条目** | 2x |
| 缓存 TTL | 3600s | **300s** | 避免过期数据 |
| 线程模型 | threadpool + Runtime | **tokio::spawn** | 零阻塞 |
| HTTP 连接 | 每次创建 | **全局连接池** | 零连接开销 |

### 优化技术

1. **移除人为延迟**：缓存命中后立即发送完整内容
2. **无限制事件处理**：每帧处理所有可用事件
3. **纯异步线程模型**：`tokio::spawn` 替代 threadpool
4. **连接池复用**：全局 HTTP_CLIENT 复用连接
5. **缓存策略优化**：缩短 TTL，增加容量
6. **延迟监控**：实时统计平均延迟

### 依赖
- tokitai: 0.3.3
- reqwest: HTTP 客户端（带连接池）
- serde_json: JSON 处理
- anyhow: 错误处理
- ratatui: TUI 框架
- tokio: 异步运行时
